...
const center: LatLngTuple = [52.22977, 21.01178];
...
const LeafletMap = () => {
  const { error: globalError, isLoading: globalLoading } = useQuery(
    "getGlobalData",
    fetchGlobalData
  );
  const {
    ...
    <div>
      ...
      >
        ...
        {countryData &&
          countryData.map(
            (
              { country, countryInfo, active, deaths, recovered }: CountryData,
              index: number
            ) => (
              <Marker
                key={index}
                position={[countryInfo.lat, countryInfo.long] as LatLngTuple}
